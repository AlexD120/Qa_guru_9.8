...
class User:
    ...
    def is_adult(self):
        return (
            self.age >= USER_ADULT_AGE
        )  # - вернет тру или фолс если работник младше 18
...
if __name__ == '__main__':  # - способ запустить код в ООП
    ...
    oleg = User(
        name='Oleg', age=16, status=Status.student, item=['pen', 'book', 'paper']
    )
    oleg2 = User(
        name='Oleg', age=16, status=Status.student, item=['pen', 'book', 'paper']
    )
    olga = User(
        name='Olga', age=22, status=Status.worker, item=['footbol', 'book', 'paper']
    )
